...
public class SettingsPanel extends JPanel {
    ...
    private java.util.List<String> currentAllowedServerIds = new java.util.ArrayList<>();
    ...
    private void initComponents() {
        ...
        stack.add(Box.createVerticalStrut(12));
        addStackCard(stack, createConfigFormCard(), 780);
    ...
    private JPanel createConfigFormCard() {
        ...
        pluginDllT2Field = addRow(form, gbc, row++, "path.plugindll.t2", "Plugin DLL 2T deploy path", null);
        pluginDllT4Field = addRow(form, gbc, row++, "path.plugindll.t4", "Plugin DLL 4T deploy path", null);
        ...
        tcServicesAndProgramsArea = addAreaRow(form, gbc, row++, "task.services.and.programs",
                "TC Windows services and executable paths to control (format: TYPE:name/path:display)", 3);
        runningTcProgramsArea = addAreaRow(form, gbc, row, "task.running.tc.programs",
                "Processes to monitor in Task Manager (comma or newline separated)", 2);

        JButton manageServersBtn = ThemeManager.createSecondaryButton("Allowed Servers");
        manageServersBtn.addActionListener(e -> manageAllowedServers());
    ...
    private JTextArea addAreaRow(JPanel form, GridBagConstraints gbc, int row,
        ...
        gbc.gridx = 1;
        gbc.weightx = 1;
        JTextArea area = ThemeManager.styleTextArea(new JTextArea(rowsCount, 40));
    ...
    private void loadIntoForm(EnvironmentConfig env) {
        ...
        currentAllowedServerIds = new java.util.ArrayList<>(env.getAllowedServerIds());
    }
    ...
    private EnvironmentConfig readFromForm() {
        ...
        env.setPathPluginDllT2(pluginDllT2Field.getText().trim());
        env.setPathPluginDllT4(pluginDllT4Field.getText().trim());
        env.setBackupDir(backupDirField.getText().trim());
        env.setDeployBackupDir(deployBackupDirField.getText().trim());
        env.setTcServicesAndPrograms(tcServicesAndProgramsArea.getText());
        env.setRunningTcPrograms(runningTcProgramsArea.getText().trim());
        env.setAllowedServerIds(new java.util.ArrayList<>(currentAllowedServerIds));
        return env;
    }
    ...
    private void manageAllowedServers() {
        List<com.cfdeploytool.model.Server> allServers = mainFrame.getServerManager().getServers();
        if (allServers.isEmpty()) {
            JOptionPane.showMessageDialog(this,
                    "No servers registered yet. Add servers in the Servers tab first.",
                    "No Servers", JOptionPane.WARNING_MESSAGE);
            return;
        }

        JDialog dialog = new JDialog((Frame) SwingUtilities.getWindowAncestor(this), "Manage Allowed Servers", true);
        ...
        JLabel title = new JLabel("Select servers allowed for this environment:");
        title.setFont(ThemeManager.FONT_BOLD);
        content.add(title, BorderLayout.NORTH);

        JPanel listPanel = new JPanel();
        listPanel.setLayout(new BoxLayout(listPanel, BoxLayout.Y_AXIS));
        listPanel.setBackground(ThemeManager.BG_CARD);
        listPanel.setBorder(new EmptyBorder(8, 8, 8, 8));

        java.util.Map<String, JCheckBox> checkboxMap = new java.util.HashMap<>();
        for (com.cfdeploytool.model.Server s : allServers) {
            JCheckBox cb = new JCheckBox(s.getName() + " (" + s.getHost() + ")");
            ThemeManager.styleCheckBox(cb);
            cb.setSelected(currentAllowedServerIds.contains(s.getId()));
            listPanel.add(cb);
            listPanel.add(Box.createVerticalStrut(6));
            checkboxMap.put(s.getId(), cb);
        }

        JScrollPane scroll = ThemeManager.styleScrollPane(listPanel);
        ...
        JButton okBtn = ThemeManager.createAccentButton("Apply");
        okBtn.addActionListener(e -> {
            currentAllowedServerIds.clear();
            for (java.util.Map.Entry<String, JCheckBox> entry : checkboxMap.entrySet()) {
                if (entry.getValue().isSelected()) {
                    currentAllowedServerIds.add(entry.getKey());
                }
            }
            dialog.dispose();
            mainFrame.setStatus("Allowed servers configured (" + currentAllowedServerIds.size() + " selected)");
        });

        JPanel actions = ThemeManager.createActionBar(cancelBtn, okBtn);

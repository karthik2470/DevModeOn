...
public class FileDeploymentService {
    ...
    public DeployResult deploy(String fileName, DeploymentCategory category,
        ...
        return deploy(fileName, category, tier, fileBytes, allowCreateDirectories, performBackup,
                existingFileBackupDir, deployingFileBackupDir, targetPathRaw, null);
    }
    ...
    public DeployResult deploy(String fileName, DeploymentCategory category,
        ...
        try {
            ...
            if (lastModified != null && lastModified > 0) {
                try {
                    Files.setLastModifiedTime(targetFile, java.nio.file.attribute.FileTime.fromMillis(lastModified));
                } catch (IOException e) {
                    System.err.println("Failed to set last modified time: " + e.getMessage());
                }
            }

...
public class VersionService {
    ...
    public VersionResult readVersion(DeploymentCategory category) {
        return readVersion(category, null);
    }

    public VersionResult readVersion(DeploymentCategory category, String targetPathRaw) {
        Path versionFile = getVersionFilePath(category, targetPathRaw);
        try {
    ...
    public VersionResult updateVersion(DeploymentCategory category, String newVersion) {
        return updateVersion(category, newVersion, true, null);
    }

    public VersionResult updateVersion(DeploymentCategory category, String newVersion,
                                       boolean allowCreateDirectories) {
        return updateVersion(category, newVersion, allowCreateDirectories, null);
    }
    ...
    public VersionResult updateVersion(DeploymentCategory category, String newVersion,
        ...
        try {
            ...
            if (targetPathRaw != null && !targetPathRaw.isBlank()) {
                deployDir = Path.of(targetPathRaw);
            } else {
                deployDir = config.getDeployPath(category, DeploymentTier.T4);
            }
    ...
    public VersionResult incrementVersion(DeploymentCategory category) {
        return incrementVersion(category, null);
    }

    public VersionResult incrementVersion(DeploymentCategory category, String targetPathRaw) {
        VersionResult current = readVersion(category, targetPathRaw);
        if (!current.isSuccess()) {
            return current;
        }
        String next = increment(current.getNewVersion());
        return updateVersion(category, next, true, targetPathRaw);
    }

    private Path getVersionFilePath(DeploymentCategory category) {
        return getVersionFilePath(category, null);
    }

    private Path getVersionFilePath(DeploymentCategory category, String targetPathRaw) {
        Path baseDir;
        if (targetPathRaw != null && !targetPathRaw.isBlank()) {
            baseDir = Path.of(targetPathRaw);
        } else {
            baseDir = config.getDeployPath(category, DeploymentTier.T4);
        }
        return baseDir.resolve(category.getVersionFileName());
    }

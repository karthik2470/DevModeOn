...
public class BackupPanel extends JPanel {
    ...
    private void initComponents() {
        ...
        infoBtn.setToolTipText("Show resolved backup paths");
    ...
    private JPanel buildFileSection() {
        ...
        JLabel searchLbl = ThemeManager.createSecondaryLabel("Filter: ");
        JTextField searchField = ThemeManager.styleTextField(new JTextField(12));
        searchField.setPreferredSize(new Dimension(100, 28));
        searchField.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            public void insertUpdate(javax.swing.event.DocumentEvent e) { filterFiles(searchField.getText()); }
            public void removeUpdate(javax.swing.event.DocumentEvent e) { filterFiles(searchField.getText()); }
            public void changedUpdate(javax.swing.event.DocumentEvent e) { filterFiles(searchField.getText()); }
        });
        ...
        btnRow.add(searchLbl);
        btnRow.add(searchField);
    ...
    private void startBackup() {
        ...
        String backupDirRaw = environmentService.getActiveEnvironment().getBackupDir();
        if (backupDirRaw == null || backupDirRaw.trim().isEmpty()) {
            JOptionPane.showMessageDialog(this,
                    "Backup directory is not configured. Go to Settings to configure corporate.backup.path.",
                    "Backup directory", JOptionPane.WARNING_MESSAGE);
            return;
        }
        final String backupDir = backupDirRaw.trim();
    ...
    private void filterFiles(String query) {
        ...
        for (JCheckBox cb : fileCheckboxes) {
            if (lower.isEmpty()) {
                ...
                cb.setVisible(cb.getText().toLowerCase().contains(lower));
            }
        }
        fileListPanel.revalidate();
        fileListPanel.repaint();
    ...
    private void showPathInfoDialog() {
        String backupDir = environmentService.getActiveEnvironment().getBackupDir();
        if (backupDir == null) {
            backupDir = "";
        }
        String message = "<html>"
                + "<h3>Backup Configuration Details</h3>"
                + "<p>These paths are configured in the active environment settings:</p>"
                + "<table border='0' cellpadding='4' cellspacing='0'>"
                + "<tr bgcolor='#F1F5F9'><td><b>2T Source Path:</b></td><td><code>" + escapeHtml(resolveSourcePath()) + "</code></td></tr>"
                + "<tr><td><b>Backup Directory:</b></td><td><code>" + escapeHtml(backupDir) + "</code></td></tr>"
                + "</table>"
                + "<br/>"
                + "<p><i>To modify these paths, please edit your environment profile in the Settings tab.</i></p>"
                + "</html>";

        JOptionPane.showMessageDialog(this, message, "Backup Path Information", JOptionPane.INFORMATION_MESSAGE);
    }

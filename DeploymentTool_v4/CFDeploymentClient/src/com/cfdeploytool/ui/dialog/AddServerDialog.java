...
public class AddServerDialog extends JDialog {
    ...
    private JCheckBox poolServerCheck;
    private JCheckBox gsServerCheck;
    private JCheckBox bgsServerCheck;
    private JCheckBox dispatcherServerCheck;
    ...
    public AddServerDialog(JFrame parent, ServerManager serverManager, Server existingServer) {
        ...
        if (existingServer != null) {
            ...
            selectServerTypes(java.util.Set.of(ServerType.OTHER));
        }
        setSize(500, 550);
    ...
    private void initComponents() {
        ...
        poolServerCheck = new JCheckBox("Pool server");
        gsServerCheck = new JCheckBox("GS server");
        bgsServerCheck = new JCheckBox("BGS server");
        dispatcherServerCheck = new JCheckBox("Dispatcher server");
        ThemeManager.styleCheckBox(otherServerCheck);
        ThemeManager.styleCheckBox(corporateServerCheck);
        ThemeManager.styleCheckBox(poolServerCheck);
        ThemeManager.styleCheckBox(gsServerCheck);
        ThemeManager.styleCheckBox(bgsServerCheck);
        ThemeManager.styleCheckBox(dispatcherServerCheck);
        otherServerCheck.setBackground(ThemeManager.BG_SURFACE);
        corporateServerCheck.setBackground(ThemeManager.BG_SURFACE);
        poolServerCheck.setBackground(ThemeManager.BG_SURFACE);
        gsServerCheck.setBackground(ThemeManager.BG_SURFACE);
        bgsServerCheck.setBackground(ThemeManager.BG_SURFACE);
        dispatcherServerCheck.setBackground(ThemeManager.BG_SURFACE);

    }

    private void selectServerTypes(java.util.Set<ServerType> types) {
        otherServerCheck.setSelected(types.contains(ServerType.OTHER));
        corporateServerCheck.setSelected(types.contains(ServerType.CORPORATE));
        poolServerCheck.setSelected(types.contains(ServerType.POOL));
        gsServerCheck.setSelected(types.contains(ServerType.GS));
        bgsServerCheck.setSelected(types.contains(ServerType.BGS));
        dispatcherServerCheck.setSelected(types.contains(ServerType.DISPATCHER));
    }

    private java.util.Set<ServerType> getSelectedServerTypes() {
        java.util.Set<ServerType> types = new java.util.LinkedHashSet<>();
        if (otherServerCheck.isSelected()) types.add(ServerType.OTHER);
        if (corporateServerCheck.isSelected()) types.add(ServerType.CORPORATE);
        if (poolServerCheck.isSelected()) types.add(ServerType.POOL);
        if (gsServerCheck.isSelected()) types.add(ServerType.GS);
        if (bgsServerCheck.isSelected()) types.add(ServerType.BGS);
        if (dispatcherServerCheck.isSelected()) types.add(ServerType.DISPATCHER);
        return types;
    }
    ...
    private void layoutComponents() {
        ...
        typePanel.setMaximumSize(new Dimension(Integer.MAX_VALUE, 100));
        ...
        JPanel checks = new JPanel(new GridLayout(2, 3, 16, 6));
        ...
        checks.add(poolServerCheck);
        checks.add(gsServerCheck);
        checks.add(bgsServerCheck);
        checks.add(dispatcherServerCheck);
    ...
    private void populateFields(Server server) {
        ...
        selectServerTypes(server.getServerTypes());
    }
    ...
    private void onSave() {
        ...
        java.util.Set<ServerType> serverTypes = getSelectedServerTypes();
        ...
        if (serverTypes.isEmpty()) {
            showValidationError("Select at least one server type.");
            return;
        }
        ...
        server.setServerTypes(serverTypes);

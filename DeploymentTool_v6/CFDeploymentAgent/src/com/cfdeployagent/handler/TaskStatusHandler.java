...
public class TaskStatusHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            String batchesRaw = JsonUtil.extractField(body, "batches");
            ...
            // Query Batch Scripts — paths delimited by "||" to avoid collisions with "," in paths
            json.append("\"batches\":{");
            if (batchesRaw != null && !batchesRaw.isBlank()) {
                String[] batches = batchesRaw.split("\\|\\|");
                boolean first = true;
                for (String batPath : batches) {
                    batPath = batPath.trim();
                    if (batPath.isEmpty()) continue;
                    if (!first) json.append(",");
                    first = false;
                    String status = taskService.getBatchStatus(batPath);
                    json.append(JsonUtil.quote(batPath)).append(":").append(JsonUtil.quote(status));
                }
            }
            json.append("}");
